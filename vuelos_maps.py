...
# %%
pasajeros_ar_df = (
    vuelos_ar_df.groupby(["origen_oaci", "destino_oaci"])["pasajeros"]
    .sum()
    .reset_index()
)
pasajeros_ar_df["coords"] = pasajeros_ar_df["destino_oaci"].map(aeropuertos_dict)
pasajeros_ar_df["latitud_destino"] = pasajeros_ar_df["coords"].apply(
    lambda x: x[0] if x is not None else None
)
pasajeros_ar_df["longitud_destino"] = pasajeros_ar_df["coords"].apply(
    lambda x: x[1] if x is not None else None
)
pasajeros_ar_df["coords"] = pasajeros_ar_df["origen_oaci"].map(aeropuertos_dict)
pasajeros_ar_df["latitud_origen"] = pasajeros_ar_df["coords"].apply(
    lambda x: x[0] if x is not None else None
)
pasajeros_ar_df["longitud_origen"] = pasajeros_ar_df["coords"].apply(
    lambda x: x[1] if x is not None else None
)
pasajeros_ar_df["aeropuerto_origen"] = pasajeros_ar_df["coords"].apply(
    lambda x: x[2] if x is not None else None
)
pasajeros_ar_df["provincia_origen"] = pasajeros_ar_df["coords"].apply(
    lambda x: x[3] if x is not None else None
)
...
fig.add_trace(
    go.Scattergeo(
        ...
        marker=dict(
            size=7,
            color="rgb(255, 0, 0)",
            line=dict(width=30, color="rgba(68, 68, 68, 0)"),
        ),
    )
)
...
for i in range(len(pasajeros_ar_df)):
    fig.add_trace(
        go.Scattergeo(
            locationmode="country names",
            lon=[
                pasajeros_ar_df["longitud_origen"][i],
                pasajeros_ar_df["longitud_destino"][i],
            ],
            lat=[
                pasajeros_ar_df["latitud_origen"][i],
                pasajeros_ar_df["latitud_destino"][i],
            ],
            mode="lines+markers",
            line=dict(width=1, color="red"),
            opacity=float(pasajeros_ar_df["pasajeros"][i])
            / float(pasajeros_ar_df["pasajeros"].max()),
        )
    )
