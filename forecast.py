...
vuelos_cordoba_df["index"] = pd.to_datetime(vuelos_cordoba_df["index"])
vuelos_cordoba_df["covid"] = np.where((vuelos_cordoba_df["index"]>= covid_start) & (vuelos_cordoba_df["index"]<= covid_end),1,0)
#%%
####################
# Train, test and validation
end_train = pd.to_datetime("01-02-2023")
end_test = pd.to_datetime("01-02-2024")
vuelos_cordoba_df = vuelos_cordoba_df.set_index("index")
...
fig = go.Figure()
fig.add_trace(go.Scatter(x=train_vuelos_cordoba_df.index, y=train_vuelos_cordoba_df["pasajeros"], mode="lines", name="Train"))
fig.add_trace(go.Scatter(x=test_vuelos_cordoba_df.index, y=test_vuelos_cordoba_df["pasajeros"], mode="lines", name="test"))
fig.add_trace(go.Scatter(x=validate_vuelos_cordoba_df.index, y=validate_vuelos_cordoba_df["pasajeros"], mode="lines", name="validation"))
fig.update_layout(title="Pasajeros domésticos de Córdoba",
...
vuelos_cordoba_df["mes"] = vuelos_cordoba_df.index.month
vuelos_cordoba_df.boxplot(column="pasajeros", by="mes", ax=axs[0], flierprops={'markersize': 3, 'alpha': 0.3})
vuelos_cordoba_df[vuelos_cordoba_df["covid"] == 0].groupby("mes")["pasajeros"].median().plot(style="o-", linewidth=0.8, ax=axs[0])
axs[0].set_ylabel('pasajeros')
axs[0].set_title('Distribución de pasajeros por mes. Ex Covid', fontsize=9)

vuelos_cordoba_df["mes"] = vuelos_cordoba_df.index.month
vuelos_cordoba_df.boxplot(column="pasajeros", by="mes", ax=axs[1], flierprops={'markersize': 3, 'alpha': 0.3})
vuelos_cordoba_df.groupby("mes")["pasajeros"].median().plot(style="o-", linewidth=0.8, ax=axs[1])
axs[1].set_ylabel('pasajeros')
axs[1].set_title('Distribución de pasajeros por mes', fontsize=9)

vuelos_cordoba_df["semana"] = vuelos_cordoba_df.index.day_of_week + 1 
vuelos_cordoba_df.boxplot(column="pasajeros", by="semana", ax=axs[2], flierprops={'markersize': 3, 'alpha': 0.3})
vuelos_cordoba_df[vuelos_cordoba_df["covid"] == 0].groupby("semana")["pasajeros"].median().plot(style="o-", linewidth=0.8, ax=axs[2])
axs[2].set_ylabel('pasajeros')
axs[2].set_title('Distribución de pasajeros por semana. Ex Covid', fontsize=9)

vuelos_cordoba_df["semana"] = vuelos_cordoba_df.index.day_of_week + 1 
vuelos_cordoba_df.boxplot(column="pasajeros", by="semana", ax=axs[3], flierprops={'markersize': 3, 'alpha': 0.3})
vuelos_cordoba_df.groupby("semana")["pasajeros"].median().plot(style="o-", linewidth=0.8, ax=axs[3])
