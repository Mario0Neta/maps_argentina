...
vuelos_cordoba_df["covid"] = np.where(
    (vuelos_cordoba_df["index"] >= covid_start)
    & (vuelos_cordoba_df["index"] <= covid_end),
    1,
    0,
)
# %%
...
train_vuelos_cordoba_df = vuelos_cordoba_df[
    (vuelos_cordoba_df.index <= end_train)
].copy()
test_vuelos_cordoba_df = vuelos_cordoba_df[
    (vuelos_cordoba_df.index > end_train) & (vuelos_cordoba_df.index <= end_test)
].copy()
validate_vuelos_cordoba_df = vuelos_cordoba_df[
    vuelos_cordoba_df.index > end_test
].copy()
print(
    f"Training will go from {train_vuelos_cordoba_df.index.min()} to {train_vuelos_cordoba_df.index.max()}"
)
print(
    f"Training will go from {test_vuelos_cordoba_df.index.min()} to {test_vuelos_cordoba_df.index.max()}"
)
print(
    f"Training will go from {validate_vuelos_cordoba_df.index.min()} to {validate_vuelos_cordoba_df.index.max()}"
)
# %%
# Data visualization
...
fig.add_trace(
    go.Scatter(
        x=train_vuelos_cordoba_df.index,
...
fig.add_trace(
    go.Scatter(
        x=test_vuelos_cordoba_df.index,
...
fig.add_trace(
    go.Scatter(
        x=validate_vuelos_cordoba_df.index,
...
plot_pacf(
    vuelos_cordoba_df[vuelos_cordoba_df["covid"] == 0]["pasajeros"], ax=ax, lags=7
)
plt.show()
# %%
fig, ax = plt.subplots(figsize=(5, 2))
