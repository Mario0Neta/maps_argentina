...
for i, vuelo in enumerate(vuelos_ar_df["vuelos"].unique()):
    ...
    print(f"The average amountof seats offered for {vuelo} flight per day is: {round(average_pasajeros)}")
    plt.bar(vuelo, average_pasajeros, alpha=0.6, label=f"Vuelos = {vuelo}", edgecolor="black")
...
vuelos_ar_df["combinaciones_provincias"] = vuelos_ar_df.apply(lambda row: tuple(sorted([row["origen_provincia"], row["destino_provincia"]])),axis=1)
