...
vuelos_ar_df.loc[:, "latitud_origen"] = vuelos_ar_df["origen_oaci"].map(
    get_coordinates(airports_dict=airports, type_of_coordinates="lat")
)
vuelos_ar_df.loc[:, "longitud_origen"] = vuelos_ar_df["origen_oaci"].map(
    get_coordinates(airports_dict=airports, type_of_coordinates="lon")
)
...
vuelos_ar_df["sorted_oaci_pair"] = vuelos_ar_df.apply(
    lambda row: tuple(sorted([row["origen_oaci"], row["destino_oaci"]])), axis=1
)
...
for row in aeropuertos_df.itertuples():
    folium.CircleMarker(
        location=[row.latitud_origen, row.longitud_origen],
        radius=5,
        color="blue",
        fill=True,
        popup=row.origen_localidad,
    ).add_to(mapa)
...
def style_function(feature):
    return {
        ...
        "weight": max(
            0.5, 100 * (feature["properties"]["pasajeros"] / total_pasajeros)
        ),
        "opacity": 0.6,
    }
...
def hover_style_function(feature):
    return {"color": "#0000FF", "weight": 5, "opacity": 0.9}  # Color when hovering
